    ...
    <div className='min-h-screen bg-background'>
      ...
      <main className='pt-20 lg:pt-24'>
        ...
        <div className='relative h-[300px] w-full overflow-hidden md:h-[400px] lg:h-[500px]'>
          <img src={post.heroImage} alt={post.heroAlt} className='size-full object-cover' />
          <div className='absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent' />
        </div>

        {/* Article Content */}
        <article className='container relative z-10 mx-auto -mt-20 px-4'>
          <div className='mx-auto max-w-3xl'>
            {/* Back Link */}
            <Link
              to={paths.blog.base}
              className='mb-6 inline-flex items-center gap-2 text-muted-foreground transition-colors hover:text-cyan'
            >
            ...
            {/* Header Card */}
            <div className='mb-8 rounded-2xl border border-border bg-card p-6 shadow-lg lg:p-10'>
              {/* Category & Read Time */}
              <div className='mb-4 flex items-center gap-3'>
                <span className='rounded-full bg-cyan/10 px-3 py-1 text-sm font-medium text-cyan'>{post.category}</span>
              ...
              {/* Title */}
              <h1 className='mb-6 text-2xl font-bold text-foreground md:text-3xl lg:text-4xl'>{post.title}</h1>

              {/* Author & Date */}
              <div className='flex items-center gap-6 border-t border-border pt-6 text-sm text-muted-foreground'>
            ...
            {/* Article Body */}
            <div className='rounded-2xl border border-border bg-card p-6 shadow-lg lg:p-10'>
              <div className='prose prose-lg max-w-none'>
                {post.sections.map((section) => {
                  if (section.type === 'intro') {
                    return (
                      <p key={section.id} className='mb-8 text-lg leading-relaxed text-foreground'>
                        {section.content}
                      </p>
                    )
                  }
                  ...
                  if (section.type === 'heading') {
                    return (
                      <div key={section.id} className='mb-8'>
                        <h2 className='mb-3 text-xl font-semibold text-foreground lg:text-2xl'>{section.title}</h2>
                  ...
                  if (section.type === 'callout') {
                    return (
                      <div key={section.id} className='mt-10 rounded-xl border-l-4 border-cyan bg-cyan/10 p-6'>
                  ...
                  if (section.type === 'image') {
                    return (
                      <div key={section.id} className='my-10'>
                        ...
                        />
                        {section.caption && (
                          <p className='mt-3 text-center text-sm italic text-muted-foreground'>{section.caption}</p>
                        )}
                      </div>
                    )
                  }
              ...
              <div className='mt-12 border-t border-border pt-8 text-center'>
                <p className='mb-4 text-muted-foreground'>Szeretné megrendelni a megfelelő szűrőket?</p>
                <Link
                  to={paths.shop}
                  className='inline-flex items-center gap-2 rounded-lg bg-cyan px-6 py-3 font-medium text-primary-foreground transition-colors hover:bg-cyan/90'
                >
                  Szűrők böngészése
                </Link>
              </div>
